...
(
    ...
    logic [15:0] y_count; // Vertical line counter
        ...
        if (!areset_n) begin
            ...
            if (axi4s_in.tvalid) begin
                if(axi4s_in.tuser)begin
                    x_count <= 1; // reset counters at SOF 
                    y_count <= 0; 
                end else if (axi4s_in.tlast)begin
    ...
    assign in_crop_region = (x_count >= LEFT_CROP_STOP) && (x_count < RIGHT_CROP_START) && (y_count < OUT_HEIGHT); // Only consider pixels within the crop region
    ...
    assign axi4s_out.tlast = x_count == (RIGHT_CROP_START - 1);   // Only assert tlast for last pixel in crop region
    assign axi4s_out.tuser = (x_count == LEFT_CROP_STOP) && tuser_p;   // Only assert tuser for first pixel in crop region
